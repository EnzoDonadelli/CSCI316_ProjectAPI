...
namespace VisaoAPI.Repositories
{
    public class FollowerRepository : IFollowerRepository
    {
        ...
        {
            const string sql = """
                SELECT FollowerId, FolloweeId, FollowedAt
                FROM Followers 
                WHERE FollowerId = @Id OR FolloweeId = @Id
                """;
        ...
        public async Task<IEnumerable<User>> GetFollowingUsersAsync(int userId)
        {
            const string sql = """
                SELECT u.UserId, u.Username, u.Email, u.FullName, u.Bio, u.ProfilePic, u.CreatedAt
                FROM Users u
                INNER JOIN Followers f ON u.UserId = f.FolloweeId
        ...
        {
            const string sql = """
                INSERT INTO Followers (FollowerId, FolloweeId, FollowedAt)
                OUTPUT INSERTED.FollowerId, INSERTED.FolloweeId, INSERTED.FollowedAt
                VALUES (@FollowerId, @FolloweeId, @FollowedAt)
                """;
